...
const regex = `groq -R {role:'system',content:'prompt(.response)'}`;
const str = `groq -L {role:'assistant',content:'prompt(!messages)'}`;
        ...
        try {
            const chatCompletion = await groq.chat.completions.create({
                messages: [
                    ...
                    { role: 'system',name:'@botFater',content: `'/'+'cmd'`},
                    {
                        role: 'system',
